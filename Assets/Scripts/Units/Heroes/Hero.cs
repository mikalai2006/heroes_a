...
public class Hero : UnitBase, IDataPlay {
    ...
    public void SetPathHero(List<GridTileNode> _path)
    {
        ...
        //}
        Data.nextPosition = path[path.Count - 1]._position;
    }
    ...
    IEnumerator MoveHero()
    {
        while (path.Count > 0 && _canMove && Data.hit >= 1)
        {
            ...
            UpdateAnimate(Position, path[0]._position);
            _animator.SetBool("isWalking", true);

            yield return StartCoroutine(SmoothLerp((Vector3)Position + moveKoof, (Vector3)path[0]._position + moveKoof));
            ...
            GameManager.Instance.mapManager.SetColorForTile(path[0]._position, Color.cyan);
            if (path[0].Protected)
            {
