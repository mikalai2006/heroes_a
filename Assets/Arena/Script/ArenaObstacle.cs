...
public class ArenaObstacle
{
    ...
    [NonSerialized] public ArenaObstacleMB ArenaObstacleMB;
    ...
    {
        GameObject.Destroy(ArenaObstacleMB);
    ...
    public virtual void LoadedAsset(AsyncOperationHandle<GameObject> handle)
    {
        if (handle.Status == AsyncOperationStatus.Succeeded)
        {
            ...
            ArenaObstacleMB = r_asset.GetComponent<ArenaObstacleMB>();
            // Debug.Log($"Spawn Entity::: {r_asset.name}");
            ArenaObstacleMB.Init(this);
        }
        else
        {
